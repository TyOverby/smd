...
module Images = struct
  let i1 =
    I.(
      string ~attr:A.(fg lightblack) "omgbbq" <-> string ~attr:A.(fg white ++ bg red) "@")
  ...
  let i5 = 0 -- 15 |> List.map (fun i -> I.pad ~t:i ~l:(i * 2) i2) |> I.zcat |> tile 5 1
  let spc = I.string " "
  ...
  let c_cube_ix =
    I.tabulate 6 1
    @@ fun r _ ->
    I.hpad 0 1 @@ I.tabulate 6 6 @@ fun b g -> I.attr A.(rgb ~r ~g ~b |> bg) spc
  ;;
...
let simpleterm ~imgf ~f ~s =
  ...
  let imgf (w, h) s =
    I.(string ~attr:A.(fg lightblack) "[ESC quits.]" <-> imgf (w, h - 1) s)
  in
  let rec go s =
