...
module JSON = struct
  let element_type e = Util.to_string (List.assoc "t" (to_assoc e))
  let element_contents e = List.assoc "c" (to_assoc e)
  ...
  let to_attr attr =
    ...
    let keyvals =
      List.map
        (function
          | [ k; v ] -> Util.to_string k, Util.to_string v
          | _ -> assert false)
        keyvals
    in
    id, classes, keyvals
  ...
  let of_attr (id, classes, keyvals) =
    ...
    let keyvals =
      `List (List.map (fun (k, v) -> `List [ `String k; `String v ]) keyvals)
    in
  ...
  and of_blocks l = `List (List.map of_block l)
  ...
  and of_inline = function
    ...
    | RawInline (f, s) -> element "RawInline" (`List [ `String f; `String s ])
    | SmallCaps i -> element "SmallCaps" (of_inlines i)
    | Space -> element_nc "Space"
    | Str s -> element "Str" (`String s)
    | UnhandledInline i -> i

  and of_inlines l = `List (List.map of_inline l)
end
...
let of_meta e =
  ...
  | "MetaInlines" ->
    MetaInlines (JSON.element_contents e |> Util.to_list |> List.map JSON.to_inline)
...
let meta_bool p k =
  ...
  | MetaBool b -> b
  | MetaString "yes" | MetaInlines [ Str "yes" ] -> true
  | MetaString "no" | MetaInlines [ Str "no" ] -> false
  | _ -> raise Not_found
;;
...
let replace_blocks f p = { p with blocks = f p.blocks }
...
let map_top_blocks f p = replace_blocks (fun blocks -> List.flatten (List.map f blocks)) p
