...
let () =
  ...
  let img1 =
    I.string ~attr "THE BLUE STRIPE ABOVE" <-> I.tabulate 1 h (fun _ _ -> I.strf "HIDDEN")
  and img2 =
    I.strf "Top line. There's a %a above. ^^^" (pp_str attr) "blue stripe" |> I.vpad 0 2
  in
  output_image img1;
  output_subst ~prev:img1 img2;
  output_image I.(string ~attr:A.(fg white) "[..]" |> eol);
  for i = 0 to 5 do
    let a = A.(bg (rgb ~r:i ~b:(5 - i) ~g:0)) in
    let bg = I.tabulate 1 i (fun _ -> I.strf "HIDDEN [%d]") |> eol
    and fg = I.(char ' ' 19 (5 - i) <|> char '-' 1 (5 - i) |> I.attr a) |> eol in
    output_image bg;
    output_subst ~prev:bg fg
  done;
  ...
  let rec go prev n =
    ...
    then (
      ...
      sleep 0.01;
      go i (n + 1))
    else output_subst ~prev I.empty
  in
  show_cursor false;
  go I.empty 1;
  show_cursor true;
  output_image
    I.(
      strf
        "It doesn't say %a anywhere on screen, either."
        (pp_attr A.(fg white) Format.pp_print_string)
        "hidden"
      |> eol)
;;
