...
let hpadwith attr c a b i = I.char ~attr c a 1 <|> i <|> I.char ~attr c b 1
...
let cuts i =
  ...
  |> List.map (fun a ->
         0 -- (w - a)
         |> List.map (fun b -> i |> I.hcrop a b |> hpadwith A.(fg lightblack) '.' a b)
         |> I.vcat
         |> I.hpad 1 1)
  |> I.hcat
  |> I.vpad 1 1
...
let () =
  let open I in
  patterns
  |> List.map (fun s -> cuts (string ~attr:A.(fg lightmagenta ++ bg lightblack) s))
  |> I.vcat
  |> eol
  |> output_image;
  colors
